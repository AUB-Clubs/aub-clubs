...
const data: string = fs.readFileSync(path.join(__dirname, "buildings.md"), "utf8");
...
async function createAndStoreEmbeddings() {
  ...
  for (const row of finalData) {
    ...
    await prisma.$executeRaw`
      INSERT INTO "building_documents" (id, content, embedding)
      VALUES (${crypto.randomUUID()}, ${row.content}, ${embeddingStr}::vector)
    `;
  }

  console.log("Inserted to building_documents");
}
